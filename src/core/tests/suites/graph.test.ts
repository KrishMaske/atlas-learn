  ...
  tests: [
    {
      ...
      fn: () => {
        ...
        store.addNode('API', { x: 100, y: 100 });
        const nodes = useGraphStore.getState().nodes;
        
        assert(nodes.length === 1, 'Should have 1 node');
        assertEqual(nodes[0].type, 'API', 'Node type should be API');
      }
    },
    {
      ...
      fn: () => {
        ...
        store.addNode('API', { x: 0, y: 0 }); // id: api-1
        store.addNode('DATABASE', { x: 100, y: 0 }); // id: db-1
