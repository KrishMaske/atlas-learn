...
import { GraphState, RestApiConfig, ContextConfig } from '@/core/types';
  ...
  tests: [
    {
      ...
      fn: () => {
        const graph: GraphState = {
          nodes: [
            { id: 'c1', type: 'CONTEXT', position: { x: 0, y: 0 }, label: 'Context', config: { context: 'Test context', files: [] } as ContextConfig },
            { id: 'api1', type: 'REST_API', position: { x: 100, y: 0 }, label: 'API', config: { capacity: 100, baseLatency: 10, errorRate: 0, method: 'GET', path: '/test' } as RestApiConfig }
          ],
        ...
        assert(ir.services.length === 1, 'Should compile 1 service (API), Context excluded');
    ...
    },
    {
      ...
      fn: () => {
        const graph: GraphState = {
          nodes: [
            { id: 'api1', type: 'REST_API', position: { x: 100, y: 0 }, label: 'API', config: { capacity: 100, baseLatency: 10, errorRate: 0, method: 'POST', path: '/api' } as RestApiConfig }
          ],
          edges: [],
          selectedNodeId: null,
          selectedEdgeId: null
